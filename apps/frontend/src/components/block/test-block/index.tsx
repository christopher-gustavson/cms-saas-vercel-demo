...
const TestBlock: CmsComponent<Partial<GraphQL.TestBlockDataFragment>> = ({
  data,
  inEditMode,
}) => {
  const {
      ...
      <div className="prose max-w-none dark:text-ghost-white">
        {overline && (
          <span
            ...
            data-epi-edit={inEditMode ? "TestBlockOverline" : undefined}
            dangerouslySetInnerHTML={{ __html: overline }}
          ></span>
        )}
        {heading && (
          <h2
            data-epi-edit={inEditMode ? "TestBlockHeading" : undefined}
            dangerouslySetInnerHTML={{ __html: heading }}
          ></h2>
        )}
        {description && description.html && (
          <span
            data-epi-edit={inEditMode ? "TestBlockDescription" : undefined}
...
const TestBlockData = gql(`
    fragment TestBlockData on TestBlock {
      overline: TestBlockOverline
s    headingSize: TestBlockHeadingSize
  s   heading: TestBlockHeadings      description: TestBlockDescription {
        json
        html
      }
      center: TestCenter
      width: TestBlockWidth
